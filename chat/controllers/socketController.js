    ...
    ws.on('message', async (message) => {
        try {
            ...
            if (result.type === 'message') {
                const savedMessage = await saveMessage(ws.id, ws.user.username, result);
                broadcastMessage(wss, ws.user, savedMessage);
            }
            else if (result.type === 'getMessages') {
                const messages = await getFormattedMessagesByGardenId(result.gardenId);
                const channels = await getFormattedChannelsByGardenId(result.gardenId);
                const initialReqObj = {
                    type: 'initialLoad',
                    messages: messages,
                    channels: channels
